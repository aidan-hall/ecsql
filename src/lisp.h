...
/* Allocate the given number of cells in memory, return its index in memory or -1 on failure. */
...
/* } */
#define OBJ_BOX(VALUE, TAG) ((Object)(((VALUE) << (OBJ_TAG_LENGTH)) | OBJ_TAG_NAME(TAG)))
...
/* } */
#define OBJ_BOX_INDEX(INDEX, METADATA, TAG) (OBJ_BOX(((INDEX) << LISP_INDEX_METADATA_LENGTH) | (METADATA), TAG))
#define OBJ_UNBOX(BOX) (BOX >> OBJ_TAG_LENGTH)
#define OBJ_UNBOX_INDEX(BOX) (BOX >> LISP_INDEX_OFFSET)
#define OBJ_UNBOX_METADATA(BOX) ((BOX & LISP_INDEX_METADATA_MASK) >> LISP_INDEX_METADATA_LENGTH)
#define OBJ_REINTERPRET(OBJ, NEWTAG) ((OBJ & OBJ_MASK) | OBJ_TAG_NAME(NEWTAG))

void wrong(const char* message);
#define LISP_ASSERT_TYPE(OBJ, TYPE) do {                        \
...
#define LISP_CAR(LISP, PAIR) (*lisp_cell_at(LISP, OBJ_UNBOX_INDEX(PAIR) + LISP_CAR_INDEX))
#define LISP_CDR(LISP, PAIR) (*lisp_cell_at(LISP, OBJ_UNBOX_INDEX(PAIR) + LISP_CDR_INDEX))
